...
class PriceBill:
    ...
    def set_prices(self, request, queryset):
        ...
        for item in queryset:
            self.prices["total"].append(item.total_price)

    def sum_prices(self):
        self.sum["total"] = round(sum(self.prices["total"]), 2)

    def write_results(self, request):
        self.success_messages.append("total price = {}".format(self.sum["total"]))
        for message in self.success_messages:
            messages.add_message(request, messages.INFO, message)

    def handle(self, request, queryset):
        self.success_messages = ["Here are the results for the {} bills selected".format(len(queryset))]
